  ...
  return (
    <div className="h-screen bg-[#FDFDFD] flex flex-col overflow-hidden font-sans">
      ...
      <main className="flex-1 flex flex-col lg:flex-row overflow-hidden">
          ...
          <section className="bg-white rounded-[2.5rem] p-6 md:p-10 shadow-[0_8px_30px_rgb(0,0,0,0.02)] border border-slate-100 space-y-8">
            ...
            <div className="hidden sm:flex p-6 md:p-8 bg-slate-900 rounded-[2rem] text-white items-center shadow-xl border border-white/5">
                <div className="flex-1">
                   <p className="text-[9px] font-black text-[#2FB8B3] uppercase tracking-[0.2em] mb-1">{t.doctor_responsible}</p>
                   <h3 className="text-xl font-bold uppercase tracking-tight leading-none">{userProfile.fullName || "---"}</h3>
                   <p className="text-[11px] opacity-50 font-medium mt-1">{userProfile.title || "Vet"}</p>
                </div>
                <div className="h-12 w-[1px] bg-gradient-to-b from-transparent via-slate-700 to-transparent mx-8"></div>
                <div className="flex-1">
                   <p className="text-[9px] font-black text-[#2FB8B3] uppercase tracking-[0.2em] mb-1">{t.clinic_center}</p>
                   <h3 className="text-lg font-bold uppercase tracking-tight leading-none">{clinicData.clinicName || "---"}</h3>
                   <p className="text-[11px] opacity-50 font-medium mt-1">{t.hq_label}</p>
                </div>
            </div>
        ...
        {/* COLUMNA IMÁGENES - Ajustada para no cortarse */}
        <div className="w-full lg:w-[45%] bg-[#F8FAFC] p-4 md:p-8 flex flex-col overflow-y-auto order-first lg:order-last min-h-[400px]">
          <div className="mb-6 px-2">
            <h3 className="font-black text-slate-800 text-2xl tracking-tight">{t.images_title} ({images.length})</h3>
            <p className="text-[11px] text-slate-400 font-bold uppercase tracking-wider">{t.images_min_req}</p>
          </div>
          ...
          <div 
            ...
            `}
          >
            <input type="file" multiple className="absolute inset-0 opacity-0 cursor-pointer z-20" onChange={handleImageUpload} />
            <div className="flex flex-col items-center text-center pointer-events-none">
              <div className="w-16 h-16 bg-[#FDFDFD] rounded-2xl shadow-md flex items-center justify-center text-[#2FB8B3] mb-4 border border-slate-50 transition-transform group-hover:scale-110">
                <span className="text-3xl font-light">+</span>
              </div>
              <p className="text-sm font-black text-slate-700 uppercase tracking-widest">{t.btn_upload}</p>
              <p className="text-[10px] text-slate-400 mt-2 font-bold uppercase">Click o arrastra tus archivos aquí</p>
            </div>
          </div>

          <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 pb-10">
            {images.map((src: any, i: number) => (
              <div key={i} className="relative aspect-square bg-white rounded-3xl overflow-hidden shadow-sm border border-slate-100 animate-in fade-in zoom-in duration-300 group">
                <img src={src} className="w-full h-full object-cover" alt="clinical evidence" />
                <button 
